#!/usr/bin/env python3
"""Web-oriented copy of tournament / player overlap checker (see tourn_check.py for CLI)."""
...
class RatingClient:
    ...
    def fetch_results(self, tournament_id: int) -> list[dict[str, Any]]:
        ...
        if not isinstance(data, list):
            raise RatingAPIError(f"Unexpected results response type: {type(data)}")
        return data
...
def run_check(
    ...
    *,
    ...
    """Run overlap check; returns report dict (JSON-serializable)."""
    ...
    def _timing_note(label: str) -> None:
        ...
        print(f"[tourn_check_web timing] {label}: {now - t_mark:.3f}s", file=sys.stderr)
        t_mark = now
    ...
    tids_sorted = sorted(all_tournament_ids)
    if parallel and len(tids_sorted) > 1:

        def _res_job(tid: int) -> tuple[int, set[int]]:
            c = _thread_local_client(base_url, timeout, verbose)
            results = c.fetch_results(tid)
            return tid, extract_participant_ids(results)

        with ThreadPoolExecutor(max_workers=parallel_workers) as pool:
            futures = [pool.submit(_res_job, tid) for tid in tids_sorted]
            for fut in as_completed(futures):
                tid, players = fut.result()
                participants_by_tournament[tid] = players
    else:
        for tid in tids_sorted:
            results = client.fetch_results(tid)
            participants_by_tournament[tid] = extract_participant_ids(results)
    _timing_note(f"fetch results with team members ({len(all_tournament_ids)} GETs)")
